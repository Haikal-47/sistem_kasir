...
import { Clock, ShieldCheck, AlertCircle, HelpCircle, X, Crown, User, KeyRound } from 'lucide-react';
import { ArfaLogo } from './ArfaLogo';

export const Header: React.FC = () => {
  const { cashier, currentUser, pendingConfirmations, setActiveTab, isSuperAdmin, setIsProfileModalOpen } = usePOS();
      ...
      <header className="h-14 bg-white border-b border-slate-200 px-3 md:px-6 flex items-center justify-between shadow-xs shrink-0 z-20">
        ...
        <div className="flex items-center gap-2 md:gap-3 min-w-0">
          ...
          <div className="flex items-baseline gap-1.5 min-w-0">
            <h1 className="text-sm md:text-base font-extrabold text-slate-900 tracking-tight truncate">
              {cashier.outletName}
            </h1>
            <span className="text-xs text-pink-600 font-extrabold hidden md:inline">ARFA FASHION</span>
          ...
          <div className="flex items-center gap-1.5">
            {isSuperAdmin ? (
              <button
                ...
                title={`Login aktif: Administrator (${currentUser?.name || 'Admin'}) • Klik untuk info akun & kata sandi`}
              >
                <Crown className="w-3.5 h-3.5 text-amber-600 shrink-0" />
                <span className="font-extrabold text-amber-900">Admin ({currentUser?.name || 'Administrator'})</span>
              ...
              <button
                ...
                title={`Login aktif: Kasir (${currentUser?.name || cashier.name}) • Klik untuk info akun`}
              >
                <User className="w-3.5 h-3.5 text-slate-500 shrink-0" />
                <span className="font-bold text-slate-800">Kasir ({currentUser?.name || cashier.name})</span>
                <span className="hidden lg:inline text-[9.5px] bg-emerald-100 text-emerald-800 border border-emerald-200 px-1.5 py-0.5 rounded font-bold uppercase tracking-wider">
                  Kasir POS
                </span>
