...
const COLOR_OPTIONS = [
  { key: 'emerald', label: 'Hijau (Emerald)', bg: 'bg-emerald-500', ring: 'ring-emerald-400' },
  { key: 'sky', label: 'Biru Muda (Sky)', bg: 'bg-sky-500', ring: 'ring-sky-400' },
  { key: 'blue', label: 'Biru (Blue)', bg: 'bg-blue-600', ring: 'ring-blue-400' },
  { key: 'violet', label: 'Ungu (Violet)', bg: 'bg-violet-600', ring: 'ring-violet-400' },
  { key: 'orange', label: 'Oranye (Orange)', bg: 'bg-orange-500', ring: 'ring-orange-400' },
  { key: 'yellow', label: 'Kuning (Yellow)', bg: 'bg-yellow-500', ring: 'ring-yellow-400' },
  { key: 'rose', label: 'Merah Muda (Rose)', bg: 'bg-rose-500', ring: 'ring-rose-400' },
  { key: 'slate', label: 'Abu-Abu (Slate)', bg: 'bg-slate-500', ring: 'ring-slate-400' },
];
...
export const PaymentMethodsPage: React.FC = () => {
  ...
  const handleSaveForm = (e: React.FormEvent) => {
    ...
    if (!cleanName) {
      setErrorMessage('Nama metode pembayaran tidak boleh kosong.');
      return;
    }
    ...
    <div className="flex-1 flex flex-col h-full bg-slate-100 overflow-hidden">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                Kelola daftar metode bayar. Metode aktif otomatis tersedia di kasir saat transaksi & checkout.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="p-6 pb-2 grid grid-cols-2 lg:grid-cols-4 gap-3 shrink-0">
        ...
        <div className="bg-white rounded-2xl p-4 border border-slate-200/80 shadow-2xs flex items-center gap-3">
          ...
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-wider text-sky-600">Tipe Transfer / E-Wallet</p>
        ...
        <div className="h-full bg-white rounded-2xl border border-slate-200/80 shadow-2xs flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto">
            <table className="w-full text-left border-collapse">
              <thead className="bg-slate-50/80 text-slate-500 text-[11px] font-bold uppercase tracking-wider sticky top-0 z-10 border-b border-slate-200">
                <tr>
                  <th className="py-3 px-5">Metode & Ikon</th>
                  <th className="py-3 px-4">Jenis Pembayaran</th>
                  <th className="py-3 px-4">Aksen Warna</th>
                  <th className="py-3 px-4 text-center">Status di Kasir</th>
                ...
                {filteredMethods.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="py-12 text-center text-slate-400">
                      ...
                      <p className="text-[11px] text-slate-400 mt-1">Coba ganti filter atau tambah metode baru.</p>
                    </td>
                  </tr>
                ) : (
                      ...
                      <tr key={method.id} className="hover:bg-slate-50/70 transition-colors group">
                        ...
                        <td className="py-3.5 px-4">
                          <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-bold ${
                            ...
                            <span>{isTunai ? 'Tunai (Langsung Lunas)' : 'Transfer / Bukti'}</span>
                          </span>
                        </td>
                        ...
                        <td className="py-3.5 px-4">
                          <div className="flex items-center gap-2">
                            <span className={`w-3.5 h-3.5 rounded-full ${
                              COLOR_OPTIONS.find(c => c.key === method.color)?.bg || 'bg-slate-400'
                            }`} />
                            <span className="capitalize text-slate-700 font-medium">{method.color}</span>
                          </div>
                        </td>
                        ...
                        <td className="py-3.5 px-5 text-right">
                          <div className="flex items-center justify-end gap-1.5">
                            ...
                            <button
                              ...
                              title={method.isDefault ? 'Metode bawaan tidak dapat dihapus' : 'Hapus Metode'}
                            >
          ...
          <div className="p-3 bg-slate-50 border-t border-slate-200 text-xs text-slate-500 flex items-center justify-between px-5">
            ...
            <span className="text-[11px] text-slate-400">
              Sinkronisasi aktif dengan database Neon PostgreSQL
            </span>
          </div>
            ...
            <div className="p-5 bg-slate-900 text-white flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  {editingMethod ? `Ubah konfigurasi metode ${editingMethod.name}` : 'Metode baru akan langsung tersedia di checkout kasir'}
                </p>
              </div>
              <button
            ...
            <form onSubmit={handleSaveForm} className="p-5 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Contoh: GoPay, ShopeePay, Bank BCA, dll."
                  className="w-full px-3 py-2.5 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:border-brand-600 outline-hidden transition-all"
                  autoFocus
                />
              </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  >
                    ...
                    <p className="text-[10px] text-slate-500 leading-tight">
                      Kasir terima uang kertas & hitung kembalian
                    </p>
                  </button>
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="font-bold text-xs">Transfer / Non-Tunai</span>
                    </div>
                    <p className="text-[10px] text-slate-500 leading-tight">
                      Transfer bank, QRIS, e-wallet dengan verifikasi bukti
                    </p>
                  </button>
                </div>
              </div>
                  ...
                  {ICON_OPTIONS.map((ico) => (
                    <button
                      ...
                      className={`w-9 h-9 rounded-xl border flex items-center justify-center text-lg transition-all ${
                        formIcon === ico
                          ? 'bg-brand-50 border-brand-500 ring-2 ring-brand-400 shadow-xs'
              ...
              <div className="pt-2 border-t border-slate-100 flex items-center justify-between">
                <div>
                  <p className="text-xs font-semibold text-slate-800">Status Aktif</p>
                  <p className="text-[10px] text-slate-500">Tampilkan metode ini di pilihan checkout kasir</p>
                </div>
                <button
                  type="button"
                ...
                >
                  {formIsActive ? <ToggleRight className="w-4 h-4 text-emerald-600" /> : <ToggleLeft className="w-4 h-4 text-slate-400" />}
              ...
              <div className="flex gap-2 pt-3">
                ...
                <button
                  type="submit"
                  className="flex-1 py-2.5 rounded-xl bg-brand-600 hover:bg-brand-700 text-white text-xs font-bold shadow-md shadow-brand-600/25 transition-all"
                >
